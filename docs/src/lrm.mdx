...
<div className="print-only browser-cta">
  [View this document in your browser](https://untangled-lang.github.io/untangled/lrm.html)
</div>
...
A [**thread procedure definition**](#thread-procedure-definitions) is a predefined sequence of
statements that can be executed in one or more threads.
...
<a id="thread-handles">A value of type **thread**</a> is an opaque
[resource handle](https://en.wikipedia.org/wiki/Handle_(computing)) to a single running instance of
a thread definition. A single **thread** runs a thread procedure corresponding to the thread
procedure definition it was spawned with.

<a id="current-thread">
  Every statement or expression being evaluated in an Untangled program “belongs” to a single
  thread, which is called the **current thread**.
</a>
...
- Square brackets indicate **character sets**. A character set contains several single-character
  ...
  <span style={{ whiteSpace: 'nowrap' }}>[<T>a</T> <T>b</T> <T>c</T>]</span> matches exactly one of
  <T>a</T>, <T>b</T>, or <T>c</T>.
  - Within a character set, a hyphen indicates an ASCII range. For example,
    <span style={{ whiteSpace: 'nowrap' }}>[<T>a</T>-<T>z</T>]</span> matches any lowercase letter.
  - A caret after the opening square brace of a character set indicates negation. For example,
    <span style={{ whiteSpace: 'nowrap' }}>[^<T>a</T>-<T>z</T>]</span> matches any character that is
...
{/*
  * DID SMT TODO: Talk about how arrays / threads are initialized with default values when
  * users declare a tuple without assignments.
  */}
{/* TODO: “These [booleans, integers, floats, strings] should have been explained in this section. I don't know how any of these types function, or what restrictions they have on them” */}
Untangled includes first-class types for booleans, integers, floats, strings,
[threads](#thread-handles), [semaphores](#semaphores), tuples, and arrays (as well as a void
type).
...
{/*
  * TODO: - Add opaque reference to semaphores
  * DID SMT TODO: - Tuple are passed by reference b.c it's a immutable
  */}
...
3. [Threads](#thread-handles) are opaque, and Untangled makes no guarantees about whether
   they are passed by value or by reference. When passed, a thread will refer to the same
   thread as before.
4. [Tuples](#nonterminal-tuple-type) are also passed by reference, however they are immutable.
...
#### Literal expressions
{/* TODO: “This definitely could have been broken down further. It's a bit hard to tease out the different sections” */}
...
{/* TODO: - Add special escape character */}
{/* TODO: - We can add double quote inside string with an escape sequence now */}
String literals start and end with a double quote that is not `\"`. The entire
contents between (not including) the double quotes make up the string value. The
following characters are special escape sequences in Untangled:
...
{/* DID SMT TODO: - Tuple literal can't store a function that returns void */}
A tuple literal groups exactly two expressions, each of which can evaluate to
any type, except the void type.

<Rule name="tuple-literal">
  <><T>(</T> <NT>expr</NT> <T>,</T> <NT>expr</NT> <T>)</T></>
</Rule>

{/* DID SMT TODO: - Array literal can't store a function that returns void */}

An array literal can have up to 2^31 - 1 values; every expressions inside an array
literal must evaluate to the same type. The only type an array cannot store is the void type.
...
variable it refers to. Trying to access the contents of an uninitialized value causes undefined behavior.
{/* TODO: “The identifier syntax would have been good to include here, rather than before. It's a bit jarring to have in two places, when you need both sections to fully understand” */}
...
For integer division, if the resulting value is not a whole value, it will be truncated.
For example `3 / 2` would yield `1`.
{/* DID SMT TODO: “How do these differ between integer and floating-point operands? For instance, what does `3 / 2` do?” */}
{/*
  * DID SMT TODO: - ** only supports integer exponent
  */}
Additionally, integer types can use the <T>\*\*</T> and <T>%</T> operators. <T>\*\*</T> raises the
left operand to the power of the right operand. <T>%</T> performs the “modulo” operation.

The comparison operators <T>&lt;</T>, <T>&lt;=</T>, <T>&gt;</T>, and <T>&gt;=</T> compare their operands and
...
{/* TODO: “it would have been good to call out specifically which types fall into which categories in this location” */}
{/*
  * TODO: - Comparison for address equality with threads / arrays / semaphores
  */}
...
{/*
  * TODO: - Explain that ++ and -- first decrement the value. This is different than what you would
  * expect in C
  */}
...
{/* TODO: “Spell this out specifically” */}
...
{/*
  * TODO: - Update this to reflect nested indexing, i.e arr[0][0]
  */}
<Rule name="array-access">
  <><NT>ident</NT> <T>[</T> <NT>expr</NT> <T>]</T></>
  <><NT>array-access</NT><T>[</T><NT>expr</NT><T>]</T></>
</Rule>
...
{/*
  * TODO: - Function call can't take void
  * TODO: - Void function return needs clarification
  */}
...
<RuleBlock>
  ...
  <Rule name="call-args">
    {/* TODO: “This feels like unnecessary granularity. You could have captured the same idea with `function-call ::= ident ( (expr (, expr)* )? )` (or just one definition for call-args).” */}
    <NT>expr</NT>
    <><NT>call-args</NT> <T>,</T> <NT>expr</NT></>
  </Rule>
</RuleBlock>

The type of each [call argument](#nonterminal-call-args) must match the type of the parameter at the
same position in the function’s [parameter list](#function-parameter-list). The number of arguments
must match as well.

{/*
  * TODO: - How does the features like parent and self relate?
  */}
Calling a function executes the sequence of statements in the function body directly in the
execution sequence of the calling thread. The “current thread” is still the thread that is executing
the function, for the purpose of features like the [<T>parent</T> and <T>self</T>
identifiers](#built-in-identifiers) and the [`receive` statement](#receive-statements).

{/* TODO: ^ “These semantics lack a lot of context... We don't even know what statements are yet. ” */}
...
<RuleBlock>
  <Rule name="variable-decl">
    <><NT>base-decl</NT>(<T>=</T><NT.expr />)?</>
    <><T>(</T><NT>tuple-decl</NT>,<NT>tuple-decl</NT><T>)</T><T>=</T><NT.expr /></>
  </Rule>
  ...
  <Rule name="tuple-decl">
    <><NT>base-decl</NT></>
    <><T>(</T><NT>tuple-decl</NT>,<NT>tuple-decl</NT><T>)</T></>
  </Rule>
</RuleBlock>

A <NT>base-decl</NT> construct binds an identifier to a type and, optionally, assigns a value to
this new variable. When a declaration includes an assignment, the type of the expression must match the
declared type of the variable.

{/* TODO - Need help explaining this */}
A <NT>tuple-decl</NT> construct is used for “tuple unpacking”—it counts as a “declaration” for each
of the “components” declared on the left hand side, and each of those declarations is assigned
the value from the corresponding component of the tuple on the right hand side.

The expression on the right side must be a tuple whose structure matches the “unpacking” on the left
hand side.
...
// Should the above be interpreted like this (1)...
if (my_boolean_1) {
  if (my_boolean_2) { print("1"); }
} else {
  print("2");
...
<T>if</T> statement (example 2 above).
...
loop. Both keywords always refer to the innermost loop that contains them.
...
{/* TODO: - Add semantics that all functions need a return statement enclosed in the top-most block */}
<Rule name="return">
  <><T>return</T> <NT>expr</NT>? <T>;</T> </>
</Rule>

The <T>return</T> keyword can only be used within a function definition.
{/* TODO: “This explanation would be better suited to be in the discussion of function definition” */}
...
{/*
  * TODO: - We can do something like (spawn thread) << 1. So the grammar should be expr << expr
  */}
A [thread](#threads) can send messages to any [thread handle](#thread-handles) it has, including the
built-in <T>parent</T> and <T>self</T> [keyword](#parent-keyword).

{/* TODO: “This explanation requires the context of how threads work, so that should come before” */}

The expression to the left of the <T>&lt;&lt;</T> must evaluate to a <T.thread /> type.

The expression to the right of the <T>&lt;&lt;</T> operator is the “message” to be sent. A message
can be a value of any type except void type. See [Message
passing](#message-passing) for an explanation of the overall semantics of the
message system.

<Rule name="message-send">
  <><NT>expr</NT> <T>&lt;&lt;</T> <NT>expr</NT> <T>;</T></>
  {/* <><T>parent</T> <T>&lt;&lt;</T> <NT>expr</NT> <T>;</T></>
  <><T>self</T> <T>&lt;&lt;</T> <NT>expr</NT> <T>;</T></> */}
</Rule>
...
{/*
  * TODO: - Receive statement does not have tuple in base pattern
  * TODO: - Receive statement does not have void as a typ
  */}
...
<RuleBlock>
  ...
  <Rule name="pattern">
    <>[<T>int</T> <T>float</T> <T>bool</T> <T>semaphore</T> <T>array-type</T>
    <T>thread</T> <T>string</T>] <NT>ident</NT></> <><T>_</T></>
    <><T>(</T> <NT>pattern</NT> <T>,</T> <NT>pattern</NT> <T>)</T> </>
  </Rule>
</RuleBlock>


A receive statement consists of an exhaustive list of <NT>pattern</NT>s, each
followed by a [statement](#nonterminal-stmt). A list of <NT>pattern</NT>s is
**exhaustive** if at least one pattern [matches](#pattern-matching-semantics)
every possible <NT>type</NT> in Untangled. Each receive statement must contain
a <T>_</T> wildcard statement and each <NT.pattern /> in a
receive statement must be unique.
...
2. Attempt to match each pattern in the <T>receive</T> statement against the message, by the
   algorithm described in [pattern matching semantics](#pattern-matching-semantics).
...
{/*
  * @DO - Mention that function-def and thread-def can share names. But thread-def by itself
  * can't share the same name. Also function-def
  */}
...
<RuleBlock>
  ...
  <Rule name="function-def-parameters">
    {/* TODO: “This also could have been simplified with regex, we do not have to act like ocamlyacc” */}
    <><NT>type</NT> <NT>ident</NT></>
...
1. The **return type** comes first; it indicates the [type](#nonterminal-type) of the value that the
  function will return. The function return type is the only part in an Untangled program that is
  allowed to have a void type.
{/* TODO: - Only within all the function names */}
2. The **name** of the function (an <NT>ident</NT>) that uniquely identifies it among all other defined
  functions and [built-in functions](#built-in-functions).
...
A function body cannot contain the following thread specific statements:
[receiving](#receive-statements) and [spawning](#thread-spawn-expressions) a new thread.
...
1. The **name** of the thread procedure uniquely identifies it among all the
  thread definitions for use with the [spawn](#thread-spawn-expressions) keyword.
2. The **body** is a [block](#blocks) that a newly spawned thread will run.
...
The Untangled program doesn't terminate until all threads have completed executing.
...
{/* TODO: “I have no idea what context is up to this point” */}
...
{/*
  * TODO: - This is true except for parent/self keywords in thread
  */}
...
`parent` and `self` are special keywords that can never be redeclared or reassign within a thread
body.
...
{/*
  * DID SMT TODO: - This is not true
  */}
Function and thread procedures are inherently different and therefore can have the same name.
However it is not possible for functions and threads to share names within themselves.
...
{/*
  * TODO: - Talk about scope of a tuple pattern?
  */}
...
- Within each thread, the <a id="parent-keyword"><T>parent</T> keyword</a> holds a thread handle to
  ...
  thread](#current-thread), i.e.
  <Code highlight className="language-untangled">self == parent</Code>.

{/* TODO: ^ “These semantics belong with other thread semantics” */}
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td><T.void/></td>
      <td>
        Prints the input string to stdout.
      </td>
    </tr>
    <tr>
      <td>`input`</td>
      ...
      <td><T.string/></td>
      <td>
        Receives the input from stdin and accepts at most 1024 characters.
      </td>
    </tr>
    <tr>
      ...
      <td><T.int/></td>
      <td><T.semaphore/></td>
      <td>Creates a semaphore with the initial value equal to the argument.</td>
    </tr>
    <tr>
      <td>`int_of_string`</td>
      <td><T.string /></td>
    ...
    <tr>
      ...
      <td><T.float/></td>
      <td>
        Returns the floating point representation of the string. The behavior is unspecified if
        the input string can't be parsed as a float.
      </td>
    </tr>
    <tr>
      <td>`bool_of_string`</td>
      <td><T.string /></td>
      <td><T.bool/></td>
      <td>
        Returns the boolean representation of a string. If the string cannot be parsed as a boolean,
    ...
    <tr>
      ...
      <td><T.int/></td>
      <td>
        Returns the floor of a float argument as an integer.
      </td>
    </tr>
    <tr>
      <td>`ceil`</td>
      ...
      <td><T.int/></td>
      <td>
        Returns the ceil of a float argument as an integer.
      </td>
    </tr>
    <tr>
      <td>`round`</td>
      ...
      <td><T.int/></td>
      <td>
        Round a float argument to the nearest integer.
      </td>
    </tr>
    <tr>
      <td>`int_of_float`</td>
    ...
    <tr>
      ...
      <td><T.int /></td>
      <td><T.void /></td>
      <td>Exits the program with the input status code.</td>
    </tr>
    <tr>
      <td>`end`</td>
      <td></td>
      <td><T.void /></td>
      <td>Ends the current thread execution. The compiler raises an error if
      `end` is used inside a function.</td>
    </tr>
  </tbody>
</table>

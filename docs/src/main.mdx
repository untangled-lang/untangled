...
## Definitions
A **thread** refers to an operating system thread; a single execution context. Threads are the
largest unit of Untangled programs.

A [**thread procedure definition**](#thread-procedure-definitions) refers to a block of Untangled
code that can be executed by one or more thread.

<a id="thread-handles">A **thread handle** refers to a value of [type](#types) <T>thread</T></a>; it
is an opaque [resource handle](https://en.wikipedia.org/wiki/Handle_(computing)) to a single running
thread.

<a id="current-thread">Every statement or
expression evaluated in an Untangled program belongs to a single thread, which is called the
**current thread**</a>.
...
It is a compiler error to declare (by a [variable declaration](#variable-declaration-statements), a
[function definition](#function-definitions), or a [thread procedure
definition](#thread-procedure-definitions)) an identifier with the same name as one of the
[keywords](#keywords).
...
There are some built-in identifiers that are not user-defined variables, but are instead keywords
...
- Within each thread, the <a id="parent-keyword"><T>parent</T> keyword</a> holds a thread handle to
  ...
  the <T>Main</T> thread, the <T>parent</T> keyword refers to the [current thread](#current-thread).
...
value](#types-passed-by-value--reference) (like strings, integers, and booleans), and perform
*reference equality* comparisons between [operands that are passed by
reference](#types-passed-by-value--reference) (like semaphores and arrays). Operands of type
<T>thread</T> ([thread handles](#thread-handles)) are equal if and only if they reference the same
thread.
...
[scope](#scope), meaning that any variables declared inside the block are only available within the
block.
...
A scope is a part of the program where a set of <NT.ident /> can be referred to.
Any reference to an <NT.ident /> that does not exist in the current scope is
a compiler error.

```untangled
int foo() { // The function body is its own scope
...
and ends at the end of the block. Each declaration in a block must have a unique
name, otherwise, it is a compiler error. Declaring a variable with the same
[identifier](#nonterminal-ident) in a nested block excludes the outer scope of
the identifier from lookup.
...
A [for loop](#for-loops) body has its own scope. *Initializer*, if present, follows the
scoping rule of [local variables](#local-variables).

```untangled
int i = 0;
for (int i = 1; i < 10; i += 2) {
  // Looked up i in the scope of the for loop
  print(i % 2 == 0);
}

int i = 0;
for (; i < 10; i += 2) {
  // Looked up i in the outer scope of the for loop
  print(i % 2 == 0);
}
```
...
Each pattern indentifier in a [receive statement](#receive-statements) is scoped to the
[statement](#nonterminal-stmt) immediately following <T.arrow />.

```untangled
receive {
  // x is scoped to the block following it
  int x -> {
    x += 5;
  }
...
### Pattern matching
Inside of a receive block, the message is matched to the corresponding receive case that matches
its type. Here is an example of pattern-matching in action:
...
thread_def Main {
  thread t = spawn patterns;
  semaphore s = make_semaphore();
  patterns << (2, (4, "hello"));
  patterns << "I'm a string";
  patterns << s;
  patterns << true;
  patterns << false;
}
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td><T.void/></td>
      <td>Prints out a human-readable representation of the data provided along with a newline in the end</td>
    </tr>
    <tr>
      <td>make_semaphore</td>
      <td>An optional <T.int/> value to set the semaphore to</td>
      <td><T.semaphore/></td>
    ...
    <tr>
      ...
      <td></td>
      <td><T.void /></td>
      <td>Exits the current thread execution</td>
    </tr>
  </tbody>
</table>

...
###### Lexer tokens
  ...
  * A monospaced, bracketed range of characters like <Code highlight>[a-z]</Code> indicates a
    ...
    * A caret `^` immediately following the opening bracket of a range
...
***Note 1**: Parentheses <code style={{ fontStyle: 'normal' }}>(</code> and
<code style={{ fontStyle: 'normal'}}>)</code> surrounding any of the above constructs indicate the
beginning and end of a **group**.*
</div>

import {
  ...
  NonTerminal as NT,
  Terminal as T,
} from "./components/Rule";

# Untangled: Language Reference Manual

<div style={{ marginTop: "-1rem", marginBottom: "3rem" }}>
  [Luke Deen Taylor](mailto:luke.taylor@tufts.edu), [Nick
  Doan](mailto:hdoan02@tufts.edu), [Duru Ugurlu](mailto:duru.ugurlu@tufts.edu),
  [Caleb Ledi](mailto:cledi01@tufts.edu), [Chloe Lam](mailto:clam08@tufts.edu)
</div>
...
receive { int x -> int result = x; * -> exit(); };
print(result);
}
...
threads, semaphores, units, tuples, and arrays.
...
The table below shows the relative precedence and associativity of operators in
Untangled. Operators with lower precedence appear in earlier rows of the table
than operators with higher precedence. Operators in the same row have the same
precedence. 
...
<Rule name="variable-decl">
  <><NT.type /><NT.ident />(<T>=</T> <NT.expr />)? <T.semi /></>
</Rule>

A variable declaration binds an identifier to a type and, optionally, assigns a
value to this new variable. When a declaration includes an assignment, the type
of the expression must match the declared type of the variable.

The compiler raises an error if the name being declared already exists in the
current <NT.block />.
...
Untangled resolves this ambiguity by associating each “else” statement with the most recent “if”
statement (i.e., the second of the two interpretations given above).

##### Loops

###### While loops

A while loop encloses a _condition_ expression and a _body_ statement. The condition expression
...
A for loop encloses three semicolon-separated parts in parentheses, followed by a _body_ statement.
We call the first part the _initialization_, the second part the _condition_, and the third part the
_afterthought_. The initialization is either a variable declaration or an expression, the condition
...
<T>break</T> jumps to the end of the loop, skipping any remaining statements in the
loop body and any remaining iterations of the loop. <T>continue</T> jumps to the
next loop iteration, skipping any remaining statements in the loop body but preserving
any remaining iterations of the loop.

<T>break</T> and <T>continue</T> both always refer to the innermost loop that contains
them:
...
###### Return Statements
The <T>return</T> keyword can only be used in within a function definition. The expression that
comes after the <T>return</T> keyword is evaluated and the evaluated value is returned to where
the function was called. Said expression must evaluate to a value of the same type as the type of
the function. The execution of a return statement finalizes the function. All functions need to
have a <T>return</T> statement. Because functions with type <T>void</T> do not return a value,
...
A thread can sent messages to any <NT.ident /> of type `thread` or its parent,
specificied by the keyword <T>parent</T>. A message is the value of the 
...
Receiving messages uses a construction with the keyword <T>receive</T>, followed by a set of
_<NT>pattern</NT>s_ mapped to [statements](#nonterminal-stmt). See
[Message passing](#message-passing) for details on the semantics of message receiving.
...
and [thread procedure definitions](#nonterminal-thread-def). Every program needs to have a <T>Main</T>
function definition that invokes other function or thread calls
...
<RuleBlock>
  ...
  <Rule name="function-def">
    <>
      <NT>type</NT> <NT>ident</NT> <T>(</T> <NT>function-def-parameters</NT>?{" "}
      <T>)</T>
      <T>{"{"}</T> <NT>stmt</NT>* <T>{"}"}</T>
    </>
  </Rule>
...
Function definitions are a way of defining a block of statements that can be added to the control
flow of a running thread. There are 4 parts to each function definition: 

1. The return type: Indicates the type of the value that will be returned by the function
2. The name: Used for identifying and refering to the function
3. The arguments: A list of type and name pairs separated by commas and enclosed in parantheses. 
These are the values that will be passed in during the function call.
4. The body of the function: A list of statements that are executed when the function is called.

Below you can see a function definition example:


```untangled
/*
Name     : min
Purpose  : Returning the smaller of two integer values
Arguments: num1, the first integer
           num2, the second integer
Return   : Returns the integer with the smallest value
*/
...
Thread definitions are blocks of code that can be assigned to run by a thread. There are 2 parts to any thread definition:

1. The name: Used for identifying the thread definition.
2. The body: The list of statements that the thread would run.

All thread definitions have access to the <T>parent</T> keyword which is a
reference to the thread that spawned the current thread. Check out the [thread
features](#thread-ft) section for more semantic information on threads. Here's
an example:
...
/*
Name     : Max
Purpose  : Sending the greater of two integer values back to the parent thread
*/
...
The way different threads communicate with each other in Untangled is through the use of message passing. 

A thread can send a message to any other thread that they have a reference for (including the parent thread) as long as they still are within scope. Sending a message to a thread that is no longer running is a runtime error. Messages sent by one thread are all sent in sequential order and are also received in sequential order (with reference to each other).

Any message sent to a thread is stored in a queue. Values sent by one thread will be also received in sequential order; however, in the case of multiple threads sending values to one thread, there is no order guarantee.

A receive block needs to be invoked in order to have access to the first message in the receiving queue. This message is then pattern matched inside of the receive block. Pattern matching happens as follows:
...
``` 
A receive block blocks the program from running until it receives a message. The pattern matching inside of a receive block should be exhaustive.
...
TODO: keywords like return, explain how functions are run as if part of the same thread, scope of functions, restrictions of function calls etc. 
...
The scope of an <NT.ident /> in a <NT.block /> begins when it is first declared
and ends at the end of the block. 

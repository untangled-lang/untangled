...
<T>break</T> jumps to the end of the loop, skipping any remaining statements in the loop body and
any remaining iterations of the loop. <T>continue</T> jumps to the next loop iteration, skipping
any remaining statements in the loop body but preserving any remaining iterations of the loop.

<T>break</T> and <T>continue</T> both always refer to the innermost loop that contains them:
...
*<NT>pattern</NT>s* mapped to [statements](#nonterminal-stmt). See
...
<RuleBlock>
  ...
  <Rule name="function-def">
    <><NT>type</NT> <NT>ident</NT> <T>(</T> <NT>function-def-parameters</NT>? <T>)</T><T>{'{'}</T> <NT>stmt</NT>* <T>{'}'}</T></>
  </Rule>

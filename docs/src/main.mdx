...
Each indentifier declared as part of a <NT>pattern</NT> in a [receive
statement](#receive-statements) belongs to the context created by the pattern matching case, which
encapsulates the [statement](#nonterminal-stmt) immediately following the <T>-></T> terminal.

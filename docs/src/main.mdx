...
following rule defines a nonterminal <NT>example-nonterminal</NT> that expands to either “hello” or
“world”:
...
String literals start and end with a double quote, and any sequence of
characters, *besides* a double quote can go inside. The entire contents between
(not including) the double quotes make up the string value.
...
<NT>infix-operator</NT> between two expressions, or a <NT>prefix-operator</NT> or
<NT>postfix-operator</NT> before or after an expression).
...
*For string operands*, the `+` operator is string concatenation.

*For boolean operands*, the `&&` and `||` operators are the logical “and” and “or” operations.

##### Prefix operators
The `-` prefix operator is negation; it applies to integer and floating-point operands.

The `!` prefix operator is the logical “not” operator; it applies to boolean operands.

##### Postfix operators

{/* TODO - Semaphores? */}
The `++` and `--` postfix operators are increment and decrement operators. They apply to integer
and floating-point operands.

##### Operator precedence and associativity
TODO: write this section

The following table provide the precedence and associativity of operators in
order from lowest to highest.

### Array access expressions
Array access expressions unpack a single value from a specified index of an array stored in a
variable. The expression inside the brackets must evaluate to an integer. The evaluated type of
...
Assignment expressions assign a value to the variable or array element that appears on the left
hand side of the expression. The right hand side of the expression must evaluate to the same type
as the variable referenced on the left (or, in the array assignment case, to the element type of the
referenced array).

The evaluated value of an assignment expression is the evaluated value of the right hand side of the
expression (i.e. the value that was assigned).
...
the group is evaluated in full before any of its components are used in any surrounding expression.
...
must be defined with a [`thread_def`](#thread_def).
...
<RuleBlock>
  <Rule name="stmt">
    ...
    {/* TODO - for loop */}
    <><T>while</T><T>&#40;</T><Expr /><T>&#41;</T><NT>stmt</NT></>
    <><T>return</T><Expr />?<Semi /></>
    <><T>break</T><Semi /></>
    <><Ident /><T>&lt;&lt;</T><Expr /><Semi /></>
  </Rule>

  <Rule name="variable-declaration">
    <><NT>type</NT><Ident /><Semi /></>
    <><NT>type</NT><Ident /><T>=</T><NT>expr</NT><Semi /></>
  </Rule>
</RuleBlock>

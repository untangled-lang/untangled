...
Untangled resolves this ambiguity by associating each else with the closest or most recent if statement.
...
The <T>break</T> keyword exits the loop, while <T>continue</T> jumps to the
next iteration of the loop. Both keywords always refer to the innermost loop that contains
them.
...
the function was called. This expression must evaluate to a value of the same type as the type of
...
a set of _<NT>pattern</NT>s_ mapped to [statements](#nonterminal-stmt). A message received matches
exactly 1 pattern based on its type and its value is bind to <T.ident />. See
[Message passing](#message-passing) for details on the semantics of message
receiving.
...
thread declaration that invokes other function or thread calls.
...
4. The body: A list of statements that are executed when the function is called.
...
features](#nonterminal-thread-def) section for more semantic information on threads. Here's
an example:

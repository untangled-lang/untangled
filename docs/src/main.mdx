...
For operators in the same row, precedence is in ascending order from left to right.
...
<tbody>
  <tr>
    <td><T>=</T> TODO: other assignment operators</td>
    <td>Right</td>
  </tr>
  <tr>
...
A variable declaration binds an identifier to a type and, optionally, assigns a value to this
new variable. When a declaration includes an assignment, the type of the expression must match the
declared type of the variable.

The compiler raises an error if the name being declared already exists in the current scope.
...
###### Return Statements
The <T>return</T> keyword can only be used in within a function definition. The expression that comes after the <T>return</T> keyword is evaluated and the evaluated value is returned to where the function was called. Said expression must evaluate to a value of the same type as the type of the function. The execution of a return statement finalizes the function. All functions need to have a <T>return</T> statement. Because functions with type <T>void</T> do not return a value, they are exempt from this rule.
...
Sending a message requires an identifier specifying the recipient (which must be a variable of type
`thread`), and an expression whose value we want to send.
...
Function definitions are a way of defining a block of statements that can be added to the control flow of a running thread.

 There are 4 parts to each function definition: the return type, the name, the arguments and the body of the function which consists of a list of statements. The return type indicates the type of the value that will be returned by the function. The name is used to identify and refer to the function. The arguments are a list of type and name pairs separated by commas and enclosed in parantheses. These names can be then used to refer to values passed on during a function call inside the body of the function. Below you can see a function definition example:
...
One thread is a singular flow of control. All statements within a singular thread are executed in sequential order from top to bottom. Any function calls that occur within the thread definition are added to the the control flow of the thread and do not execute in parallel. 

Each thread has a <T>parent</T> thread, which is a keyword that can be used to reference the "parent" thread that spawned the current thread. This keyword can be used in place of a thread identifier. It is a runtime error(TODO: can a parent thread go out of scope before a child thread?) to reference a parent thread that has terminated and gone out of scope.

A variable declared in the scope of one thread will go out of scope once the thread stops its execution. 

A thread needs to be spawned with an existing [thread definition](#nonterminal-thread-def). A spwaned thread can operate without a stored reference and can continue operating even if the parent thread has finished operating (TODO: this wholly depends on what we decid for operations with parent threads).

#### Main Thread features

Every program needs to have a <T>Main</T> thread definition which is the starting point of any Untangled program. The program starts by spawning a thread with the <T>Main</T> thread definition and starts running the procedure defined in it. More threads with the <T>Main</T> thread definition can be spawned using the <T>spawn</T> keyword.

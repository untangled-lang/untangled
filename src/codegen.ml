...
let translate ((tdecls : sthread_decl list), (fdecls : sfunc_decl list)) =
  ...
  in let build_func_body fdecl =
    ...
    let (final_builder, _) = stmt (builder, StringMap.empty) (SBlock fdecl.sbody) in
    ()
  ...
  in let _ = List.map build_thread_body tdecls
  in let _ = List.map build_func_body fdecls
in the_module
